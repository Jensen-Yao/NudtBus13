...
// site-A Chinese labels are lossy-mojibake'd at fetch time; times/structure are
...
function parseSiteA() {
  ...
  for (const c of chunks) {
    ...
    if (!known) throw new Error(`Unknown site-A service signature: ${sig}`);
    lines.push({
      line: known.line,
      dayProfile,
      origin,
...
function parseSiteB() {
  ...
  // site-B createService 使用中文站点名 —— 统一映射到英文 stop id
  const B2_STOP_ALIAS = {
  ...
  for (const dayKey of ["everyday", "monThu", "friday", "saturday", "sunday"]) {
    ...
    for (const c of dayM[1].split("createService(").slice(1)) {
      ...
      out.schedules[dayKey].push({ line, origin, dest, departures: times, offsets });
    }
  }
  ...
  for (const [tag, block] of sightBlocks) {
    for (const c of block.split("createService(").slice(1)) {
      ...
      out.sightseeing.push({ line, origin, dest, dayType: tag, departures: times, offsets });
    }
  }

  // crowd rules（dorm / college 两段分别解析，避免串段）
  const crowdBlock = extractBetween(md, "const CROWD_RULES = {", "\nconst DORM_WALK_LINES");
  out.crowdRules = { dorm: {}, college: {} };
  const dormPart = extractBetween(crowdBlock, "dorm: {", "college:");
  const collegePart = extractBetween(crowdBlock, "college: {", "}");
  for (const [name, part] of [["dorm", dormPart], ["college", collegePart]]) {
    for (const m of part.matchAll(/(mild|high): \[([^\]]*)\]/g)) {
      out.crowdRules[name][m[1]] = [...m[2].matchAll(/"(\d{2}:\d{2})"/g)].map((x) => x[1]);
    }
  }
...
function parseOfficialCampus() {
  ...
  for (const m of md.matchAll(/\| (线路\d) \| ([^|]+)\| ([^|]+)\| ([^|]*)\|/g)) {
    ...
    rows.push({ line, dir, times, note: m[4].trim() });
  }
  return rows;
}
...
function normalize(siteA, siteB, official) {
  const services = [];
  const add = (s) => services.push(s);

  // -- site-A: 8号线 + 4 college shuttles --
  for (const l of siteA.lines) {
    ...
    add({
      ...
      trips: l.departures.map((t) => ({ depart: t, days: DAYS[l.dayProfile] })),
    });
  }

  // -- site-B 环线1路 + 就餐专线 (everyday) --
  for (const s of siteB.schedules.everyday || []) {
    ...
    add({
      ...
      extraNote: "",
      trips: s.departures.map((t) => ({ depart: t, days: DAYS.all7 })),
    });
  }

  // -- site-B 线路2/5/7/8 (inter-campus) --
  const B2LINES = {
  ...
  for (const dayKey of ["monThu", "friday", "saturday", "sunday"]) {
    for (const s of siteB.schedules[dayKey] || []) {
  ...
  for (const row of official) {
    ...
    if (OFF_NEW.has(row.line)) {
      add({
        ...
        trips: row.times.map((t) => ({ depart: t.time, days: DAYS[t.day] || DAYS.monFri })),
      });
    } else {
      const out = to === "three";
  ...
  for (const [key, svc] of Object.entries(mergedB2)) {
    add({
      ...
      trips: [...svc.trips.values()].map((t) => ({ depart: t.depart, days: [...t.days] })),
    });
  }

  // -- sightseeing 环线3路 --
  for (const s of siteB.sightseeing) {
    ...
    add({
      ...
      trips: s.departures.map((t) => ({ depart: t, days: s.dayType === "weekend" ? DAYS.weekend : DAYS.weekday })),
    });
  }
...
// ---------- merge ----------
function main() {
  const nudt = parseSiteA();
  const bus2 = parseSiteB();
  ...
  console.log(`  site-A lines: ${nudt.lines.length}`);
  console.log(`  site-B rows: ${Object.values(bus2.schedules).flat().length} + sightseeing ${bus2.sightseeing.length}`);
