...
const COLLEGE\_OFFSET\_SPECIAL\_MINUTES = 7;
const STOP\_STORAGE\_KEY = "bus-stop-preference";
const UPDATE\_NOTICE\_STORAGE\_KEY = "bus-update-notice-2026-06-20-v6";
const UPDATE\_NOTICE\_DISMISSED\_VALUE = "dismissed";
const UPDATE\_NOTICE\_EXPIRES\_AT = new Date(2026, 5, 28, 0, 0, 0, 0).getTime();
...
const HOLIDAY\_DATES\_2026 = new Set(\[\
...
const DORM\_WALK\_LINES = new Set(\["线路2", "线路5", "线路7", "线路8"\]);
const ORIGIN\_VISIBLE\_LINES = new Set(\["环线1路", "环线3路(观光车)", "就餐专线v2"\]);
const CIRCULAR\_ROUTE\_LINES = new Set(\["环线1路", "环线3路(观光车)"\]);
const ADDITIONAL\_STOP\_IDS = new Set(\[\
  "eastGate",\
  "northGate",\
  "militaryCenter",\
  "laserInstitute",\
  ...
  "secondCanteen",\
\]);
const DORM\_WALK\_NOTICE = "提醒：这班车的上车点不在宿舍楼下，从宿舍出发需要先步行到对应站点。";
...
// production.
const LOOP\_ONE\_ADDITIONAL\_STOP\_OFFSETS = {
  eastGate: 1,
  militaryCenter: 3,
  laserInstitute: 5,
  northGate: 6,
...
const LOOP\_THREE\_FROM\_DORM\_ADDITIONAL\_STOP\_OFFSETS = {
  militaryCenter: 3,
  laserInstitute: 4,
  gaochaoSouth: 6,
  scienceCollege: 7,
...
const LOOP\_THREE\_FROM\_COLLEGE\_ADDITIONAL\_STOP\_OFFSETS = {
  scienceCollege: 1,
  secondCanteen: 4,
  militaryCenter: 12,
  laserInstitute: 13,
...
const DINING\_ADDITIONAL\_STOP\_OFFSETS = {
  scienceCollege: 1,
  secondCanteen: 5,
};
...
const WEEKEND\_HOLIDAY\_SIGHTSEEING\_SERVICES = \[\
  ...
  \], {\
    dorm: 0,\
    college: 5,\
    ...LOOP\_THREE\_FROM\_DORM\_ADDITIONAL\_STOP\_OFFSETS,\
  }),\
  ...
  \], {\
    college: 0,\
    dorm: 7,\
    ...LOOP\_THREE\_FROM\_COLLEGE\_ADDITIONAL\_STOP\_OFFSETS,\
  }),\
\];

const WEEKDAY\_SIGHTSEEING\_SERVICES = \[\
  createService("环线3路(观光车)", "宿舍", "系统楼", \[\
    "07:35", "07:45", "07:55",\
...
const SCHEDULES = {
  everyday: \[\
    ...
    \], {\
      dorm: 0,\
      college: COLLEGE\_OFFSET\_SPECIAL\_MINUTES,\
      ...LOOP\_ONE\_ADDITIONAL\_STOP\_OFFSETS,\
    }),\
    ...
    \], {\
      college: 0,\
      ...DINING\_ADDITIONAL\_STOP\_OFFSETS,\
    }),\
  \],
  monThu: \[\
...
function shouldShowUpdateNotice() {
  return Date.now() < UPDATE\_NOTICE\_EXPIRES\_AT
    && window.localStorage.getItem(UPDATE\_NOTICE\_STORAGE\_KEY) !== UPDATE\_NOTICE\_DISMISSED\_VALUE;
}
...
function dismissUpdateNoticePermanently() {
  window.localStorage.setItem(UPDATE\_NOTICE\_STORAGE\_KEY, UPDATE\_NOTICE\_DISMISSED\_VALUE);
  hideUpdateNotice();
}
...
function readInitialStop() {
  ...
  const storedStop = window.localStorage.getItem(STOP\_STORAGE\_KEY);
...
function isWeekendOrHoliday(date) {
  const day = date.getDay();
  return day === 0 || day === 6 || HOLIDAY\_DATES\_2026.has(formatDateKey(date));
}
...
function getServicesForDate(date) {
  ...
  const weekdaySightseeingServices = isWeekendOrHoliday(date)
    ? \[\]
    : WEEKDAY\_SIGHTSEEING\_SERVICES;
  const weekendHolidayServices = isWeekendOrHoliday(date)
    ? WEEKEND\_HOLIDAY\_SIGHTSEEING\_SERVICES
    : \[\];
...
function getCrowdLevel(lineLabel, stopId, departureTime) {
  ...
  const stopRules = CROWD\_RULES\[stopId\];
  if (!stopRules) {
    return null;
  }
...
function buildTrip(service, departure, date, stopId) {
  ...
  return {
    ...
    routeLabel: CIRCULAR\_ROUTE\_LINES.has(service.lineLabel) && ADDITIONAL\_STOP\_IDS.has(stopId)
...
function buildDepartureText(trip) {
  if (ORIGIN\_VISIBLE\_LINES.has(trip.lineLabel)) {
...
function getWalkWarning(trip, stopId) {
  ...
  return DORM\_WALK\_LINES.has(trip.lineLabel) ? DORM\_WALK\_NOTICE : "";
}
...
async function handleFeedbackSubmit(event) {
  ...
  try {
    await window.emailjs.send(EMAILJS\_CONFIG.serviceId, EMAILJS\_CONFIG.templateId, {
      ...
      query\_mode: state.queryMode === "manual" ? "手动时间" : "按现在",
...
function setSelectedStop(stopId) {
  ...
  window.localStorage.setItem(STOP\_STORAGE\_KEY, stopId);
  render();
}
